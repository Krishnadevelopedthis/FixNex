...
class WhatWebAdapter(ScannerAdapter):
    ...
    def run(self, ctx: ScanContext) -> ScanResult:
        ...
        aggression = "3" if ctx.profile == ScanProfile.COMPREHENSIVE else "1"
        args = [binary, "-a", aggression, "--log-json=-", "--no-errors", ctx.url]
        result.command_summary = " ".join(args)
        ctx.progress(f"Fingerprinting {ctx.url} with WhatWeb", 25)

        completed = run_command(args, timeout=min(ctx.timeout, 180))
        result.exit_code = completed.exit_code
        result.tool_version = tool_version(settings.WHATWEB_PATH, "--version")

        if completed.timed_out:
            result.error = completed.stderr
            return result
        if not completed.stdout.strip():
            result.error = (completed.stderr or "WhatWeb produced no output.")[:400]
            return result

        ctx.progress("Parsing WhatWeb output", 80)
        result.findings, technologies = self._parse(ctx, completed.stdout)
        result.metrics = {"technologies": technologies}
        return result

    def _parse(self, ctx: ScanContext, output: str) -> tuple[list[NormalizedFinding], list[dict]]:
        ...
        for entry in entries:
            ...
            target_url = entry.get("target", ctx.url)
            for plugin_name, data in plugins.items():
                ...
                technologies.append(
                    {"name": plugin_name, "version": version, "category": "WhatWeb"}
                )
                if plugin_name in _NOTABLE_PLUGINS:
                    severity, cwe, vector, description, remediation = _NOTABLE_PLUGINS[plugin_name]
                    findings.append(
                        NormalizedFinding(
                            title=f"{plugin_name.replace('-', ' ')} detected by WhatWeb",
                            ...
                            cwe=cwe,
                            cvss_vector=vector,
                            evidence=truncate(json.dumps(data), 1000),
